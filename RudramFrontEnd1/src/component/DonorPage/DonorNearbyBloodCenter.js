...
const NearbyBloodCenters = () => {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (selectedCity) {
      fetchBloodCenters(selectedCity);
    }
  };

  const fetchBloodCenters = async (cityId) => {
    try {
      const response = await axios.get(`http://localhost:8383/api/blood-centers`, {
        params: {
  ...
  return (
    ...
    <div className="nearby-blood-centers-container">
      <h1>Find Nearby Blood Centers</h1>
      ...
      {bloodCenters.length > 0 && (
        <div className="blood-centers-list">
          <h2>Near By Blood Centers</h2>
          <ul>
            {bloodCenters.map(center => (
              <li key={center.id}><h6>Blood Center Name={center.name}</h6> <h6>Blood Center Address={center.address}</h6>
...
export default NearbyBloodCenters;
